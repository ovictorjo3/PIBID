...
\title[Função Logarítmica - Parte I]{Função Logarítmica - Parte I}
\institute[]{% 
...
\section{Propriedades operatórias}
	...
	\begin{frame}{Aplicação}
		...
		\begin{alertblock}{\exercicio}
			\justifying
			Qual é a expressão $\beta$ cujo desenvolvimento logarítmico (em base 10) é $\log\beta = 1+\log\text{a}+2\log\text{b}-\log\text{c}$, com a, b e c números reais e positivos?
		\end{alertblock}
...
\section{Exercícios Complementares}
	...
	\begin{frame}{Cederj 2023}
		Se $2^{x}=\dfrac{1}{3}$, então o valor de $x$ é igual a: 
		\vspace{0.5cm}
	...
	\begin{frame}{UFRGS 2020}
		Se $\log2=x$ e $\log3=y$, então $\log288$ é:
	...
	\begin{frame}{EEAR-SP 2019}
		Sejam $m$, $n$ e $b$ números reais positivos, com $b \neq 1$. Se $\log_{b}m=x$ e se $\log_{b}n=y$, então $\log_{b}(m.n)+\log_{b}\left(\dfrac{n}{m} \right) $ é igual a:
	...
	\begin{frame}{UFRGS 2019}
		O valor de $$E=\log\left(\dfrac{1}{2} \right)+\log\left(\dfrac{2}{3} \right)+...+\log\left(\dfrac{999}{1000} \right) \ \textbf{é:} \ $$ 
	...
	\begin{frame}{Fesp - SP}
		O logaritmo de 32 na base $\sqrt[5]{0,5}$ é:
	...
	\begin{frame}{Fuvest - SP}
		Se $x^{5}=1000$ e $b^{3}=100$, então o logaritmo de x na base b vale:
	...
	\begin{frame}{Fuvest - SP}
		Se $\text{x}=\log_{4}7$ e $\text{y}=\log_{16}49$, então $x-y$ é igual a:
	...
	\begin{frame}{Fuvest - SP}
		Sabendo-se que $5^{p}=2$, podemos concluir que $\log_{2}100$ é igual a:
	...
	\begin{frame}{Fuvest - SP}
		Se $\log_{10}8=\text{a}$, então $\log_{10}5$ vale:
